...
public class Espectador {
	...
	public List<Midia> buscarGenero(Genero genero) {
		return assistirFuturo.stream().filter(s -> s.getGenero() == genero).collect(Collectors.toList());
	}
	...
	public List<Midia> buscarIdioma(Idioma idioma) {
		return assistirFuturo.stream().filter(s -> s.getIdioma() == idioma).collect(Collectors.toList());
	}
	 ...
	 * @throws EspectadorAvaliaException Se ocorrer um erro ao verificar se o
	 *                                   espectador pode avaliar a mídia
	 */
	...
	public void setTipoEspectador(TipoEspectador tipoEspectador) {
	 ...
	 /**
     * Assiste a uma mídia específica, registrando a visualização caso ela esteja na lista de assistir no futuro.
     *
     * @param midiaAssistir A mídia a ser assistida.
     */
    public void assistirMidia(Midia midiaAssistir) {
        if (assistirFuturo.contains(midiaAssistir)) {
            assistirFuturo.remove(midiaAssistir);
            registrarVisualizacao(midiaAssistir);
        } else {
            System.out.println("A mídia não está na lista de assistir no futuro.");
        }
    }

    /**
     * Obtém a quantidade de mídias já assistidas.
     *
     * @return A quantidade de mídias já assistidas.
     */
    public int getQuantidadeMidiasAssistidas() {
        return jaAssistida.size();
    }

    /**
     * Obtém a nota atribuída a uma determinada mídia.
     *
     * @param midia A mídia para a qual se deseja obter a nota.
     * @return A nota atribuída à mídia. Retorna -1 se a mídia não foi avaliada.
     */
    public double getNota(Midia midia) {
        for (Avaliacao avaliacao : listaNotas) {
            if (avaliacao.getMidia().equals(midia)) {
                return avaliacao.getNota();
            }
        }
        return -1; // Valor de retorno para indicar que a mídia não foi avaliada
    }
}